...
def find_obj_in_module(module, relative_obj_name):
    obj = module
    # print('---226tfutil--- %s ----  %s ------'%(module,relative_obj_name))
    for part in relative_obj_name.split('.'):
        obj = getattr(obj, part)
    return obj

def import_obj(obj_name): # metrics.frechet_inception_distance.API  # metrics.inception_score.API
    # print('---232--- %s ------'%obj_name)
    module, relative_obj_name = import_module(obj_name) # model 有问题
