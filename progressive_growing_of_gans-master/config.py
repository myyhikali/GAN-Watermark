...
tf_config = EasyDict()  # TensorF low session config, set by tfutil.init_tf().
...
#desc += '-cifar10_slime600_mullabel';             dataset = EasyDict(tfrecord_dir='cifar10_slime600_mullabel'); train.mirror_augment = True
#desc += '-cifar10_wm3000_mullabel';             dataset = EasyDict(tfrecord_dir='cifar10_wm3000_mullabel'); train.mirror_augment = True
# desc += '-simple_faces_9W_128';             dataset = EasyDict(tfrecord_dir='simple_faces_9W_128'); train.mirror_augment = True
desc += '-simple_faces_9W_WM_128';             dataset = EasyDict(tfrecord_dir='simple_faces_9W_WM_128'); train.mirror_augment = True
...
desc += '-preset-v2-1gpu'; num_gpus = 1; sched.minibatch_base = 4; sched.minibatch_dict = {4: 128, 8: 128, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8, 512: 4}; sched.G_lrate_dict = {1024: 0.0015}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 25000
# desc += '-preset-v2-2gpus'; num_gpus = 2; sched.minibatch_base = 8; sched.minibatch_dict = {4: 256, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8}; sched.G_lrate_dict = {512: 0.0015, 1024: 0.002}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 30000
#desc += '-preset-v2-4gpus'; num_gpus = 4; sched.minibatch_base = 16; sched.minibatch_dict = {4: 512, 8: 256, 16: 128, 32: 64, 64: 32, 128: 16}; sched.G_lrate_dict = {256: 0.0015, 512: 0.002, 1024: 0.003}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 25000
...
desc += '-fp32'; sched.max_minibatch_per_gpu = {32: 128 , 64: 64 , 128: 32 , 256: 16, 512: 8, 1024: 4}
...
train = EasyDict(func='util_scripts.evaluate_metrics', run_id=14, log='metric-fid-50k.txt', metrics=['fid'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
#train = EasyDict(func='util_scripts.evaluate_metrics', run_id=11, log='metric-is-50k.txt', metrics=['is'], num_images=50000, real_passes=1); num_gpus = 1; desc = train.log.split('.')[0] + '-' + str(train.run_id)
