...
class WGAN_GP(object):
    ...
    def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir, mode):
        ...
        if dataset_name == 'mnist' or dataset_name == 'fashion-mnist':
            ...
            self.z_dim = z_dim          # dimension of noise-vector
            self.y_dim = 25             # dimension of condition-vector (label)
            self.c_dim = 3              # dimension of channel
    ...
    def build_model(self):
        ...
        """ Graph Input """
        # images
        self.inputs = tf.placeholder(tf.float32, [bs] + image_dims, name='real_images') # shape = [128,64,64,3]
        print ('[Tip Hung-yi Lee] Data Augmentation (flip images horizontally + rotate images)') # 数据增强 Data Augmentation
        ...
        D_fake = self.discriminator(G, self.y, is_training=True, reuse=tf.AUTO_REUSE) # 这里应该是D根据fake图片生成的分数
        # Output of D for real images.
        D_real = self.discriminator(images_rotated, self.y, is_training=True, reuse=tf.AUTO_REUSE)

        # Output of D for wrong images.
        D_wrong_img = self.discriminator(self.imgs_wrong, self.y, is_training=True, reuse=tf.AUTO_REUSE) #imgs_wrong ？
        ...
        # get loss for discriminator 关于D判别的损失
        ...
        # get loss for generator 关于G判别的损失
    ...
    def train(self):
        ...
        self.test_labels = np.zeros((self.batch_size, 25))
        # 以下是输出的测试图，标签被我改过了，每轮输出的测试图已经改过了
        # blue hair - red eyes
        for i in range(5):
        ...
        # green hair - red eyes
        for i in range(5):
            self.test_labels[10 + i][4] = 1
            self.test_labels[10 + i][22] = 1
        # green hair - brown eyes
        for i in range(5):
            self.test_labels[15 + i][4] = 1
            self.test_labels[15 + i][21] = 1
        # no hair - no eyes
        for i in range(5):
            self.test_labels[20 + i][12] = 1
            self.test_labels[20 + i][24] = 1
        ...
        # restore check-point if it exits 如果之前有训练就载入继续训练，如果没有就从头训练
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.num_batches):
                ...
                # update D network
                for _ in range(self.d_iters): # d_iters = 2
                ...
                for _ in range(self.g_iters): # g_iters = 1
    ...
    def infer(self, test_labels):
        ...
        if could_load:
            ...
            save_images(samples[:image_frame_dim * image_frame_dim, :, :, :], [image_frame_dim, image_frame_dim],
                        check_folder('./samples') + '/' + 'cgan'+ now +'.png')
            print(" [*] Load SUCCESS")
        else:
            print(" [!] Load failed...")
