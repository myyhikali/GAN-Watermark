...
def D_logistic_r1(G, D, opt, training_set, minibatch_size, reals, labels, gamma=10.0):
    _ = opt, training_set
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])  # 维度[minibatch_size,512]的正态分布值
    fake_images_out = G.get_output_for(latents, labels, is_training=True) # 根据G生成fake图片
    
    real_scores_out = D.get_output_for(reals, labels, is_training=True)
    fake_scores_out = D.get_output_for(fake_images_out, labels, is_training=True)
    
    real_scores_out = autosummary('Loss/scores/real', real_scores_out) # autosummary表示输入组件的平均值
    fake_scores_out = autosummary('Loss/scores/fake', fake_scores_out)
