...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        #myy改的with tfutil.absolute_variable_scope(self.scope, reuse=False), tfutil.absolute_name_scope(self.scope):  # ignore surrounding scopes
        with tfutil.absolute_variable_scope(self.scope, reuse=tf.AUTO_REUSE), tfutil.absolute_name_scope(self.scope):  # ignore surrounding scopes
            #print('--153-----调用absolute_variable_scope')
    ...
    def __getstate__(self) -> dict:
        ...
        state["variables"]          = list(zip(self.vars.keys(), tfutil.run(list(self.vars.values()))))
        return state
